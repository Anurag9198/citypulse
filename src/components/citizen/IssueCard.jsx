...
export const IssueCard = ({ issue, onUpvote, onSelect }) => {
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-2xl bg-slate-900/80 border border-slate-800/80 shadow-md backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-slate-700 hover:shadow-xl hover:shadow-blue-500/10">
      ...
      >
        ...
        {/* Ticket ID Tag */}
        <div className="absolute bottom-3 left-3 bg-slate-950/80 backdrop-blur-md px-2.5 py-1 rounded-md text-[11px] font-mono text-slate-300 border border-slate-800 shadow-sm">
          #{issue.id}
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-auto pt-4 flex items-center justify-between border-t border-slate-800/60">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-semibold transition-all duration-300 hover:scale-105 active:scale-95 ${
              issue.hasUpvoted
                ? 'bg-blue-500/20 text-blue-400 border border-blue-500/40 shadow-[0_0_15px_rgba(59,130,246,0.15)]'
                : 'bg-slate-800/80 text-slate-300 border border-slate-700/80 hover:bg-slate-700 hover:text-white hover:border-slate-600 shadow-sm'
            }`}
          >
            <ThumbsUp className={`h-3.5 w-3.5 transition-transform ${issue.hasUpvoted ? 'fill-blue-400 text-blue-400 scale-110' : ''}`} />
